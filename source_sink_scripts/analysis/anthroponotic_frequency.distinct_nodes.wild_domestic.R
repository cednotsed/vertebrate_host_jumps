...
jump_df <- fread("results/ancestral_reconstruction_out/host_jump_lists/like2.diff_hosts.genus_counts.all_jumps.V2.csv") %>%
  ...
  filter(!(anc_genus %in% domestic & tip_genus %in% domestic))

zoo_df <- jump_df %>%
  mutate(event_type = ifelse(anc_genus %in% domestic, "Domestic->Wild", "Wild->Domestic")) %>%
  mutate(host = ifelse(event_type == "Domestic->Wild", anc_state, tip_state))
...
# Add t test results
t_test <- t.test(zoo_boot_df$`Domestic->Wild` , zoo_boot_df$`Wild->Domestic`,
                 paired = T, 
...
ggsave("results/source_sink_analysis/domestic_to_wild_frequency.pdf", 
       dpi = 600, 
       width = 4,
       height = 3)
