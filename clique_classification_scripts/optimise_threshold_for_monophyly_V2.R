...
fam_morsels <- foreach(fam = unique(to_do$family)) %do% {
  ...
  tree <- read.tree(str_glue("data/trees/viral_family_subsets/{fam}.220723.mash_dist.nwk"))
  ...
  mono_morsels <- foreach(threshold = seq(0, 0.5, 0.05)) %do% {
    ...
    tibble(family = fam, t = threshold, 
           mono = sum(clus_morsels), 
           total = length(clus_morsels),
           prop_mono = prop_mono)
  }
