...
jump_df <- fread("results/mutational_load_out/putative_host_jumps.csv") %>%
  ...
  filter(!(clique_name %in% c("Orthomyxoviridae_1", 
                              "Orthomyxoviridae_2",
                              "Coronaviridae_12")))
...
plot_df %>%
  ...
  geom_vline(xintercept = log(obs_df$obs_prop),
             color = "red",
             lty = "dashed") +
  geom_text(x = 0.5, y = 75, label = str_glue("p={pval}"),
            color = "red")

ggsave("results/source_sink_analysis/structured_permutation_test.no_SARS_Flu.png")
