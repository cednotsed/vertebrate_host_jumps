...
# Viral diversity data
div_df <- fread("results/metagenomic_out/pipeline_out/06_abundance_matrix/abundance_matrix.C.S.tsv")
...
genome_meta <- fread("results/species_classification_out/final_cluster_metadata.140423.csv")

human_cliques <- genome_meta %>%
  filter(host_genus == "Homo") %>%
  distinct(cluster)

parsed <- div_df %>%
  pivot_longer(!run, names_to = "viral_clique", values_to = "read_count") %>%
  filter(read_count > 0) %>%
  filter(viral_clique %in% human_cliques$cluster) %>%
  left_join(meta) %>%
  filter(librarystrategy %in% c("WGS", "RNA-Seq")) %>%
...
rep_df <- genome_meta %>%
  group_by(host_genus) %>%
  summarise(n_cliques_rep = n_distinct(cluster)) %>%
  arrange(desc(n_cliques_rep))

diversity_df <- parsed %>%
  mutate(host_genus = capitalize(host_genus)) %>%
  group_by(host_genus) %>%
  summarise(n_cliques = n_distinct(viral_clique),
            log_sum_bases = log10(sum(bases))) %>%
  left_join(rep_df) %>%
  mutate(n_cliques_rep = ifelse(is.na(n_cliques_rep), 0, n_cliques_rep),
         log_cliques_rep = log10(n_cliques_rep + 1)) %>%
  arrange(desc(n_cliques))

# Correct for clique representation and sequencing effort
# Visualise confounders
diversity_df %>%
  select(-host_genus, -n_cliques_rep) %>%
  pivot_longer(!n_cliques, names_to = "confounders", values_to = "value") %>%
  mutate(confounders = case_when(confounders == "log_cliques_rep" ~ "log10(clique representation)",
                                 confounders == "log_sum_bases" ~ "log10(sum bases sequenced)")) %>%
...
ggsave("results/modelling_out/zoonotic_diversity/confounders_linear_visualisation.png", width = 6, height = 4)

# Get residuals
diversity_lm <- lm(log10(n_cliques) ~ log_cliques_rep + log_sum_bases,
                   data = diversity_df)
...
predictors <- predictors[!(predictors %in% c("host_genus", "host_family", "host_order",
                                             ...
                                             "log_sum_bases", "n_cliques_rep", "corr_diversity"))]
...
ggsave("results/modelling_out/zoonotic_diversity/bootstrap_MAE.png", width = 8, height = 5)
...
tibble(x = perm_morsels) %>%
  ...
  geom_text(aes(x = obs_dev_explained + 0.05, 
            ...
            color = "red") + 
  theme_classic2() +
  labs(x = "Deviance explained", y = "No. of permutations")

ggsave("results/modelling_out/viral_diversity/permutation_test.png", width = 8, height = 5)
