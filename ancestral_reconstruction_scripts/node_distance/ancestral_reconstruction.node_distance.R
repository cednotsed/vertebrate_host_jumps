...
res_dir <- "results/ancestral_reconstruction_out/node_distance/ancestral_reconstructions.node_distance"
...
dir.create(str_glue("{res_dir}/reconstructed_trees"))
...
foreach(tree_path = tree_paths,
            ...
            if (fold_diff > 2) {
              ...
              if (anc_state != "Unknown") {
                if (anc_state != tip_state & 
                  ...
                  # Get node name where host state is same as tip state
                  if (init_node == tip_name) {
                    init_name <- tip_name
                  } else {
                    init_name <- scaled_tr$node.label[init_node - Ntip(tree)]                    
                  }
                  ...
                  # Get ancestral node name where host jump occured
                  node_name <- scaled_tr$node.label[parent_name - Ntip(tree)]
                  
                  return(tibble(anc1_name = init_name,
                                anc2_name = node_name,
                                tip_name = tip_name, 
                                anc_state = anc_state,
                                tip_state = tip_state,
                  ...
                  # Change initial node if node state is the same
                  init_node <- parent_name
                }
              }
            }
          }
  ...
  ggtree(rooted, 
    ...
    geom_inset(pies, 
               width=0.03, 
               height=0.03) +
    geom_nodelab(size = 3, vjust = 1) +
    geom_tiplab(size = 3) +
    labs(color = "Host")
  
  ggsave(str_glue("{res_dir}/reconstructed_trees/{clique_name}.anc_recon.pdf"),
         width = 15, height = 15)
