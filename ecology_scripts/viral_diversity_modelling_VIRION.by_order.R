...
# Analyse by host order
order_counts <- merged_df %>%
  group_by(host_order) %>%
  summarise(n_genera = n_distinct(host_genus)) %>%
  arrange(desc(n_genera)) %>%
  head(3)
  
for(order_name in order_counts$host_order) {
    merged_filt <- merged_df %>%
      filter(host_order == order_name)
  ...
  perm_morsels <- foreach(i = seq(perm_iters), 
                          .combine = "c") %do% {
    temp_dat <- merged_filt %>%
      mutate(corr_diversity = sample(merged_filt$corr_diversity, replace = F))
  ...
  tibble(x = perm_morsels) %>%
    ...
    labs(x = "Deviance explained", 
         y = "No. of permutations", 
         title = str_glue("Permutation test p = {p}, n_genera = {n_distinct(merged_filt$host_genus)}"))
  
  ggsave(str_glue("results/modelling_out/viral_diversity/virion_data/permutation_test.{order_name}.png"), width = 8, height = 5)
}
