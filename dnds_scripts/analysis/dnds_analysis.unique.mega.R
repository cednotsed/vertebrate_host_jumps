...
dnds_df <- fread("results/dnds_out/dnds_out.diff_hosts.genus_counts.mega.csv") %>%
...
res_filt <- dnds_df %>% 
  # filter(!is.na(ka)) %>%
  mutate(ks = ifelse(ks == 0 | ks < 0, 0.00000001, ks),
         ka = ifelse(ka == 0, 0.00000001, ka)) %>%
...
res_filt %>%
  # filter(n_accs > 5) %>%
  group_by(n_hosts, is_jump) %>%
  summarise(var_kaks = min(kaks)) %>%
  ggplot(aes(x = n_hosts, y = var_kaks, color = is_jump)) +
  geom_point() +
  geom_smooth(method = "loess")
clique_counts <- res_filt %>%
  filter(cds_length > 5000) %>%
  ungroup() %>%
  group_by(n_hosts) %>%
...
plot_df %>%
  ggplot(aes(x = factor(n_hosts), y = log10(kaks))) +
...
ggsave("results/dnds_out/dnds_visualisation.mega.png", width = 5, height = 5)

summary(lm(kaks ~ cds_length, data = plot_df %>% filter(is_jump)))
lr1_jump <- glm(n_hosts ~ cds_length + kaks,
                data = plot_df %>% filter(is_jump),
                family = poisson())

lr2_jump <- glm(n_hosts ~ cds_length + log(kaks),
                data = plot_df %>% filter(is_jump),
                family = poisson())

lr3_jump <- glm(n_hosts ~ log(cds_length) + log(kaks),
                data = plot_df %>% filter(is_jump),
                family = poisson())

lr4_jump <- glm(n_hosts ~ log(cds_length) + kaks,
                data = plot_df %>% filter(is_jump),
                family = poisson())
