...
jump_df <- fread("results/mutational_load_out/host_jump_lists/diff_hosts.genus_counts.csv")
...
fix_annot <- function(gene_aln, gene_pos, full_aln) {
  ...
  if (max(positions_fixed) > ncol(full_aln)) {
    positions_fixed <- gene_pos[1:ncol(full_aln) - remainder]
  }
...
morsels <- foreach(i = seq(nrow(jump_df)), .packages = c("data.table",
                                                         "tidyverse",
                                                         "MSA2dist",
                                                         "Biostrings",
                                                         "foreach")) %do% {
 # i = 7024
 # clique_name <- "Filoviridae_8"
 # tip <- "KY785985.1"
 # node_name <- "Node120"
 ...
 if(nrow(genes) > 0) {
   ...
   gene_morsels <- foreach(annot_string = annot_list) %do% {
     # annot_string = annot_list[174]
     # Get gene annotation
     ...
     if (length(strand_val) == 1) {
       ...
       if (!is.null(gene_fixed)) {
         concat_seq <- apply(gene_fixed, 1, paste0, collapse = "")
         concat_seq <- DNAStringSet(concat_seq)
         print(str_glue("{ncol(gene_fixed) %% 3 == 0}"))
         
         tryCatch({
           dnds <- dnastring2kaks(concat_seq, 
                                model = "Li")
           ...
           return(tibble(cluster = clique_name, 
                         ...
                         tip_state = tip_state,
                         is_jump = is_jump,
                         gene_name = gene_annot,
                         gene_length = ncol(gene_fixed),
                         ka = ka,
                         ks = ks))
           
         }, error = function(err) {
           print(str_glue("for {tip_name}, dnds for {gene_annot} failed!"))
           return(NULL)
         })
       } else {
         return(NULL)
...
fwrite(res, "results/dnds_out/dnds_out.diff_hosts.genus_counts.by_gene.csv")
