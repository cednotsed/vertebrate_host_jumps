...
cov_df %>%
  group_by(is_jump, gene_type) %>%
  summarise(prop_ks_zeroes = sum(ks_zero) / n(),
            prop_ka_zeroes = sum(ka_zero) / n()) %>%
  mutate(ratio = prop_ks_zeroes / prop_ka_zeroes) %>%
  # select(is_jump, prop_ks_zeroes, prop_ka_zeroes, gene_type) %>%
  # pivot_longer(!c(is_jump, gene_type), names_to = "type", values_to = "estimate") %>%
  ggplot(aes(x = gene_type, y = ratio, fill = is_jump)) +
  # facet_grid(.~ type) +
