...
for (virus in c("IBV", "PDCoV", "MERS", "SC2")) {
  # virus <- "IBV"
  ...
  fna <- readDNAStringSet(file_path)
  ...
  plot_df %>%
    ...
    ggplot(aes(x = domain, y = log_kaks, fill = is_jump)) +
    geom_boxplot() +
    theme_classic() + 
  ...
  ggsave(str_glue("results/dnds_out/by_domain/{virus}_spike.pdf"),
         dpi = 600,
         height = 4, width = 8)
}
