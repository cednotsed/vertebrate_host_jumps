...
# Remove zeroes
dnds_filt <-  dnds_df %>%
  mutate(ks = ifelse(ks <= 0, 0, ks),
         ka = ifelse(ka <= 0, 0, ka)) %>%
  mutate(ka_zero = ifelse(ka == 0, T, F),
         ks_zero = ifelse(ks == 0, T, F)) %>%
  filter(!(ka == 0 & ks == 0)) 
...
dnds_df %>%
  ...
  mutate(ka_zero = ifelse(ka == 0, T, F),
         ks_zero = ifelse(ks == 0, T, F)) %>%
  group_by(is_jump) %>%
  summarise(prop_ks_zeroes = sum(ks_zero) / n(),
            prop_ka_zeroes = sum(ka_zero) / n()) %>%
  select(is_jump, prop_ks_zeroes, prop_ka_zeroes) %>%
  pivot_longer(!is_jump, names_to = "type", values_to = "estimate") %>%
  ggplot(aes(x = is_jump, y = estimate)) +
  facet_grid(.~ type) +
...
dnds_df %>%
  ...
  mutate(ka_zero = ifelse(ka == 0, T, F),
         ks_zero = ifelse(ks == 0, T, F)) %>%
  group_by(is_jump, clique_name, n_hosts) %>%
  summarise(prop_ks_zeroes = sum(ks_zero) / n(),
            prop_ka_zeroes = sum(ka_zero) / n()) %>%
  select(is_jump, clique_name, n_hosts, prop_ks_zeroes, prop_ka_zeroes) %>%
  pivot_longer(!c(is_jump, clique_name, n_hosts), names_to = "type", values_to = "estimate") %>%
  ggplot(aes(x = is_jump, y = estimate, fill = is_jump)) +
  facet_grid(.~ type) +
  geom_boxplot()
...
wilcox <- wilcox.test(log10(merged_df$kaks) ~ merged_df$is_jump,
                      alternative = "less")

merged_df %>%
  summarise(range(min_kaks))
